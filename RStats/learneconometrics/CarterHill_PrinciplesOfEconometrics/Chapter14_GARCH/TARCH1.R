## SOURCE: 
# https://faculty.chicagobooth.edu/ruey.tsay/teaching/introTS/Tgarch11.R
...
TGARCH11 <- function(x,cond.dist="norm")
{
      ...
      if(cond.dist=="std"){
            params = c(mu = Meanx, omega = 0.1*Varx, alpha = 0.1, gam1= 0.02, beta = 0, shape=6)
            lowerBounds = c(mu = -10*abs(Meanx), omega = S^2, alpha = S, gam1=S, beta = 0, shape=3)
            upperBounds = c(mu = 10*abs(Meanx), omega = 100*Varx, alpha = 1-S, gam1 = 1-S, beta = 0, shape=30)
      }else{
            params = c(mu = Meanx, omega = 0.1*Varx, alpha = 0.1, gam1= 0.02, beta = 0)
            lowerBounds = c(mu = -10*abs(Meanx), omega = S^2, alpha = S, gam1=S, beta = 0)
            upperBounds = c(mu = 10*abs(Meanx), omega = 10*Varx, alpha = 1-S, gam1 = 1-S, beta = 0)
      }
